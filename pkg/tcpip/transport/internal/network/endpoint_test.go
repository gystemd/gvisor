...
func TestEndpointSimple(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if err := ep.Write(func(netProto tcpip.NetworkProtocolNumber, src, dst tcpip.Address, maxHeaderLength int, _ bool) (*stack.PacketBuffer, tcpip.Error) {
				if netProto != test.expectedNetProto {
					t.Errorf("got netProto = %d, want = %d", netProto, test.expectedNetProto)
				}
				if src != test.expectedLocalAddr {
					t.Errorf("got src = %s, want = %s", src, test.expectedLocalAddr)
				}
				if dst != test.expectedRemoteAddr {
					t.Errorf("got dst = %s, want = %s", dst, test.expectedRemoteAddr)
				}

				return stack.NewPacketBuffer(stack.PacketBufferOptions{ReserveHeaderBytes: maxHeaderLength, Data: data.ToVectorisedView()}), nil
			}, tcpip.WriteOptions{}); err != nil {
