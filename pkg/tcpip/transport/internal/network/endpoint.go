...
func (e *Endpoint) prepareWrite(pktf PacketFunc, opts tcpip.WriteOptions) (writeContext, tcpip.Error) {
	e.mu.RLock()
	defer e.mu.RUnlock()
	...
	pkt.Owner = e.owner
	return writeContext{route: route, pkt: pkt, ttl: calculateTTL(route, e.ttl, e.multicastTTL), tos: e.sendTOS}, nil
}

// Write writes data to the endpoint's peer. This method does not block if
// the data cannot be written.
func (e *Endpoint) Write(f PacketFunc, opts tcpip.WriteOptions) tcpip.Error {
	// MSG_MORE is unimplemented. This also means that MSG_EOR is a no-op.
	if opts.More {
		return &tcpip.ErrInvalidOptionValue{}
	}

	ctx, err := e.prepareWrite(f, opts)
	if err != nil {
		return err
	}
	defer ctx.route.Release()

	if e.ops.GetHeaderIncluded() {
		return ctx.route.WriteHeaderIncludedPacket(ctx.pkt)
	}

	return ctx.route.WritePacket(stack.NetworkHeaderParams{
