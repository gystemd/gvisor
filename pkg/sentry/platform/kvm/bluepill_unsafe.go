	...
	for {
		...
		case _KVM_EXIT_MMIO:
			...
			if mmio.isWrite != 0 {
				...
				_, sig = safecopy.Memcpy(virtual, uintptr(unsafe.Pointer(&mmio.data[0])), uintptr(mmio.length))
			} else {
				_, sig = safecopy.Memcpy(uintptr(unsafe.Pointer(&mmio.data[0])), virtual, uintptr(mmio.length))
			}
			if _, _, errno := unix.RawSyscall6(
